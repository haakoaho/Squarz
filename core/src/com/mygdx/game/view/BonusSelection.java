...
public class BonusSelection extends State {
    ...
    protected BonusSelection(GameStateManager gsm, PreferencesSettings settings, CountDown countDown, Bonus bonus) {
        super(gsm);
    ...
    public void chosenBonus(int x, int y){
        ...
        if (none.contains(x, y)){
            this.setBonusSelected(this.none.getTexture());
            this.emptyField.setTexture(bonusSelected);
            this.bonus.setBonusKey(0);
            this.bonus.setColorKey(3);
        }
        if (killer.contains(x, y)){
            this.setBonusSelected(this.killer.getTexture());
            this.emptyField.setTexture(bonusSelected);
            this.bonus.setBonusKey(1);
            this.bonus.setColorKey(4);
        }
        if (multi.contains(x, y)){
            this.setBonusSelected(this.multi.getTexture());
            this.emptyField.setTexture(bonusSelected);
            this.bonus.setBonusKey(2);
            this.bonus.setColorKey(5);
        }
    }
